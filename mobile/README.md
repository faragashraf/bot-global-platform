...
ENPO Connect migration Slice 4 adds ENPO-isolated Firebase initialization, FCM destination lifecycle, application-scoped push registration, semantic notification parsing, durable inbox/deduplication, Android presentation, and localized inbox/detail navigation. It reuses the shared Firebase runtime, registration controller, semantic push processor, URL policy, and notification inbox contract. The ENPO Firebase configuration, approved URL host, legacy channel namespace, preferences, and six notification sounds remain product-local. Pairing completes before push activation; Firebase or registration failures never roll back a durable paired credential. The profile capability reads a minimized, already-published Bot Global projection through the paired device credential and never calls Connect V2 directly; it uses no durable mobile profile cache. ENPO's visible cold launch retains the legacy three-second cinematic logo choreography; background and process-dead Firebase startup does not create the Compose launch surface. App Protection and full unpair remain intentionally absent.

Provision `EnpoConnectMobile/androidApp/google-services.json` from the ENPO Firebase project locally or in CI before Android builds. The file is ignored and must not be committed or replaced with the NQRB configuration. Android channels retain the legacy `enpo_connect_notifications_{normal|high}_{sound}` IDs, including stable IDs for all six packaged ENPO sounds. Existing channels are reused because Android keeps channel importance and sound immutable after creation.
